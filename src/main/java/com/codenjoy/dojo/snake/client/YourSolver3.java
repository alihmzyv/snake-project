        ...
        if (isOnDeadPoint(point)) {
            ...
            return allEmptyNeighbours.stream()
                    ...
                    .max(/*(path1, path2) -> {
                        ...
                        else if(path1 == path2 && path1 == null) {
                            return 0;
                        }
                        ...
                        logger.printf(Level.INFO, "Path to empty neighbour of the point: %s. Lenght: %d Weight: %d",
                                path.getVertexList(),
                                path.get);
                        int weight = (int) path.getWeight();
                        return weight;*/
                    Comparator.comparingInt(path -> {
                        GraphPath<Point, DefaultEdge> pathC = (GraphPath<Point, DefaultEdge>) path;
                        return - pathC.getLength();
                    }).thenComparingInt(path -> {
                        GraphPath<Point, DefaultEdge> pathC = (GraphPath<Point, DefaultEdge>) path;
                        return (int) pathC.getWeight();
                    }))
                    .map(path -> {
                        List<Point> vertexList = path.getVertexList();
                        logger.printf(Level.INFO, "Path to empty neighbour of the point, decided: %s", vertexList);
