...
public class YourSolver3 implements Solver<Board> {
    ...
    private Optional<Point> getNextPoint() {
        ...
        if (pointChosen.isEmpty()) {
            pointChosen = getDirToFurthestEmptyPointNoStone(10);
            if (pointChosen.isEmpty()) {
                pointChosen = getDirToFurthestEmptyPointWithStone(10);
                if (pointChosen.isEmpty()) {
    ...
    public Optional<Point> getDirToPoint(Point point) {
        ...
        if (getNeighbours(point, obstacles).size() == 1 &&
                !getNeighbours(point, obstacles).contains(head)) {
            logger.printf(Level.INFO, "The apple or stone is on the dead point. Not going..");
            return Optional.empty();
        }
    ...
    private List<Point> getNeighbours(Point point, List<Point> barriers) {
        return PointHelper.getNeighbours(point, dw, dh, boardSize, barriers, false);
    }
    ...
    private void addWeightToEmptyPoints() {
        emptyPoints
                .forEach(point -> getNeighbours(point, barriers)
                        .forEach(neighbourPoint -> {
